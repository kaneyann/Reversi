...
void get_input_com(int board[][SIZE], int player, int *p, int *q);                              // 打ち手の探索と決定(COM)
int store_legal(int board[][SIZE], state store[], int player);                                  // 全ての合法手の位置を格納する
int count_total_turn_over(int board[][SIZE], state store[], int player, int count, int pos);    // 挟んだ石の数を返す(走査方向は全方向)
...
{
    state store[LAST*LAST];    // 合法手格納用の配列
    int ct_legal;              // 合法手の個数
    int ct_turn_over;          // 各合法手における、裏返せる石の個数
    int max = 0;               // 最も多く裏返せる石の個数
    int i;
    ...
    for ( i = 0; i < ct_legal; i++ ) {
        ct_turn_over = count_total_turn_over(board, store, player, ct_turn_over, i);    // 各合法手における、裏返せる石の個数を計算
        if ( ct_turn_over > max ) {    // 裏返せる石の個数がmaxよりも多いならば
            max = ct_turn_over;
            *p = store[i].p;
            *q = store[i].q;
        }
...
int store_legal(int board[][SIZE], state store[], int player)
{
    ...
    for ( i = FIRST; i <= LAST; i++ ) {
        for ( j = FIRST; j <= LAST; j++ ) {
            if ( is_legal_move(board, player, i, j) == TRUE ) {    // 合法手ならば
                store[ct].p = i;
                store[ct].q = j;
                ct++;
            }
...
{
    int d, e;    // 走査する方向を決める変数(d:上下方向、e:左右方向)
    
    count = 0;
    for ( d = DEC_RAW; d <= INC_RAW; d++ ) {         // 上下方向
        for ( e = DEC_COL; e <= INC_COL; e++ ) {     // 左右方向
            count += count_turn_over(board, player, store[pos].p, store[pos].q, d, e);    // 裏返せる個数を1方向ごとに加算
        }
    }
...
void game(void)
{
    ...
    while ( 1 ) {
        ...
        if ( player == BLK ) {
            get_input_human(board, player, &p, &q);    // 打ち手の入力(人間)
        } else if ( player == WHT ) {
            get_input_com(board, player, &p, &q);      // 打ち手の入力(COM)
        }
