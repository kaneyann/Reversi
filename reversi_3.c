...
// 人間vsCOMのリバーシゲーム
// AI : ミニマックス法
// 盤面評価の方法 : 白石(COMの石)の数がそのまま評価値となる
//==========================================================
...
#define SEARCH_LEVEL 5    // 先読みするレベル

//--------------------------------------------------------------------
//  大域宣言
//--------------------------------------------------------------------

int copy[SEARCH_LEVEL][SIZE][SIZE];

//--------------------------------------------------------------------
//  大域定義
//--------------------------------------------------------------------

enum { EMP = 0, BLK = 1, WHT = -1, OUTSIDE = -100 };    // マス目の状態
...
int value_board(int board[][SIZE])
{
    ...
    count_stone(board, &cb, &cw);    // 両プレイヤーの石の個数を数える
